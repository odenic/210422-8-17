<template>
  <div class="detail" v-if="res.price">
    <!-- 商品分类导航 -->
    <TypeNav />
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span>{{ res.categoryView.category1Name }}</span>
        <span>{{ res.categoryView.category2Name }}</span>
        <span>{{ res.categoryView.category3Name }}</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        <!-- 左侧放大镜区域 -->
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :imgUrl="getZoomImg" />
          <!-- 小图列表 -->
          <ImageList
            :imgUrl="res.skuInfo.skuImageList"
            :changeZoomImg="changeZoomImg"
          />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in res.spuSaleAttrList" :key="index">
...
export default defineComponent({
  ...
  methods: {
    async reqGoodsDetail(): Promise<void> {
      try {
  ...
  computed: {
    getZoomImg(): string {
      ...
      if (this.res.skuInfo) {
        return this.res.skuInfo.skuDefaultImg;
      }
      return "";
    },
  },
